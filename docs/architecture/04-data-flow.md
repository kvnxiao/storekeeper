...
Timeline:
- **T+0ms**: Tauri app starts, config loaded, state initialized
...
`reload_config()` recreates both `GameClientRegistry` and `DailyRewardRegistry` from the new config, then triggers an immediate refresh.
...
7. Frontend TypeScript types
   AllResources { games: Record<GameId, GameResource[]>, lastUpdated: string }
       │
       ▼
8. Jotai atom selectors extract per-game resources
       │
       ▼
9. React component renders StaminaCard with countdown timer
```
