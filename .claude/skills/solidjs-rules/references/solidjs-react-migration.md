...
Solid's JSX looks like React, but its execution model differs: components run once and updates flow
through fine-grained signals, not re-renders. The table maps React habits that break Solid
reactivity to Solid patterns. The other rules in this pack cover the patterns in depth.
...
Default migrations to `eslint-plugin-solid`. Its `reactivity`, `no-destructure`, `prefer-for`,
`components-return-once`, `no-react-deps`, and `no-react-specific-props` rules catch the
incompatible React patterns in this table. Keep an existing lint stack when it enforces equivalent
checks.
